...
int main() {
  ...
  while(!quit) {
    ...
    camera.x = (theDot.getPosX() + dot::DOT_WIDTH  / 2) - SCREEN_WIDTH  / 2;
    camera.y = (theDot.getPosY() + dot::DOT_HEIGHT / 2) - SCREEN_HEIGHT / 2;
    ...
    //Render background
    g_bgTexture.render(0, 0, &camera);

    //Render objects
    theDot.render(camera.x, camera.y);
